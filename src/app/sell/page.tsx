...
const DEMO_OFFERS: Offer[] = [
  { id: "s1", username: "buymaster", user_stats: { completion_rate: 99, completed_trades: 2840, is_verified_trader: true }, payment_methods: ["UPI", "Paytm"], price_inr: 86.80, min_limit_inr: 500, max_limit_inr: 50000, available_usdt: 3000 },
  { id: "s2", username: "inrbuyer", user_stats: { completion_rate: 98, completed_trades: 1920, is_verified_trader: true }, payment_methods: ["Bank Transfer", "IMPS"], price_inr: 86.75, min_limit_inr: 1000, max_limit_inr: 200000, available_usdt: 10000 },
  { id: "s3", username: "quickpay", user_stats: { completion_rate: 96, completed_trades: 760, is_verified_trader: false }, payment_methods: ["PhonePe", "UPI"], price_inr: 86.60, min_limit_inr: 300, max_limit_inr: 15000, available_usdt: 600 },
  { id: "s4", username: "premiumdesk", user_stats: { completion_rate: 97, completed_trades: 1240, is_verified_trader: true }, payment_methods: ["NEFT", "IMPS"], price_inr: 86.50, min_limit_inr: 2000, max_limit_inr: 500000, available_usdt: 25000 },
];
...
export default function SellPage() {
  ...
  const filteredOffers = amountFilter
    ? offers.filter((o) => { const amt = parseFloat(amountFilter); return !isNaN(amt) && amt * o.price_inr >= o.min_limit_inr && amt * o.price_inr <= o.max_limit_inr; })
    : offers;
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <Navbar />
      <main className="max-w-7xl mx-auto px-4 py-8 w-full flex-1" data-testid="sell-page">

        <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
          <div>
            <h1 className="font-black text-2xl text-foreground">Sell USDT</h1>
            <p className="text-sm text-muted-foreground mt-0.5">Choose a buyer and sell your USDT for INR</p>
          </div>
          <Link href="/buy" className="text-sm text-muted-foreground hover:text-foreground transition-colors flex items-center gap-1">
            Want to buy? <ArrowRight className="size-3.5" />
          </Link>
        </div>
        ...
        <div className="flex flex-wrap gap-3 mb-6 p-4 bg-card border border-border rounded-xl">
          <div className="relative">
            ...
            <input type="number" value={amountFilter} onChange={(e) => setAmountFilter(e.target.value)} placeholder="Enter USDT amount..." className="pl-9 pr-4 py-2 bg-surface border border-border rounded-lg text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:outline-none w-48" />
          </div>
          <div className="relative">
        ...
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          ...
          {loading ? (
            <div>{[1,2,3,4].map((i) => <div key={i} className="px-5 py-4 border-b border-border last:border-0 animate-pulse"><div className="h-4 bg-surface rounded w-1/3" /></div>)}</div>
          ) : filteredOffers.length === 0 ? (
            <div className="text-center py-16">
              <p className="text-muted-foreground mb-3">No offers found</p>
              <Link href="/create-offer" className="text-primary text-sm font-semibold hover:underline">Post a sell offer</Link>
            </div>
            ...
            <div key={offer.id} className={`grid grid-cols-1 md:grid-cols-[2fr_1.5fr_1.5fr_1.5fr_1fr] px-5 py-4 gap-3 md:gap-0 items-center table-row-hover transition-colors ${i !== filteredOffers.length - 1 ? "border-b border-border" : ""}`} data-testid={`offer-row-${offer.id}`}>
              <div className="flex items-center gap-3">
                <div className="size-9 rounded-full bg-success/15 flex items-center justify-center text-success text-sm font-bold shrink-0">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">{offer.user_stats?.completed_trades ?? 0} trades &middot; {offer.user_stats?.completion_rate ?? 0}% rate</p>
                </div>
              </div>
              <div>
                <p className="font-stat font-bold text-lg text-success">₹{offer.price_inr.toFixed(2)}</p>
              ...
              <div className="flex flex-wrap gap-1.5">
                {offer.payment_methods?.slice(0, 3).map((m) => <span key={m} className="px-2 py-0.5 bg-surface border border-border text-muted-foreground text-[11px] rounded font-medium">{m}</span>)}
                {(offer.payment_methods?.length ?? 0) > 3 && <span className="px-2 py-0.5 text-[11px] text-muted-foreground">+{(offer.payment_methods?.length ?? 0) - 3}</span>}
              </div>
              <div className="md:text-right">
                <button onClick={() => { setSelectedOffer(offer); setTradeAmount(""); setTradePayment(offer.payment_methods?.[0] || ""); }} className="px-5 py-2 bg-success text-success-foreground text-sm font-bold rounded-lg hover:opacity-90 transition-opacity" data-testid={`sell-btn-${offer.id}`}>
                  Sell
                </button>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" data-testid="trade-modal">
          <div className="bg-card border border-border rounded-2xl w-full max-w-md shadow-2xl shadow-black/60">
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              <div>
                <h3 className="font-bold text-foreground">Sell USDT</h3>
                <p className="text-xs text-muted-foreground mt-0.5">to {selectedOffer.username}</p>
              </div>
              <button onClick={() => setSelectedOffer(null)} className="p-1.5 rounded-lg hover:bg-surface transition-colors"><X className="size-4 text-muted-foreground" /></button>
            </div>
            <div className="px-6 py-5 flex flex-col gap-4">
              <div className="bg-surface rounded-xl p-4 flex flex-col gap-2 text-sm">
                <div className="flex justify-between"><span className="text-muted-foreground">Rate</span><span className="font-stat font-semibold text-success">₹{selectedOffer.price_inr}/USDT</span></div>
              ...
              <div>
                ...
                <div className="relative">
                  <input type="number" step="0.01" value={tradeAmount} onChange={(e) => setTradeAmount(e.target.value)} className="w-full px-4 py-3 bg-surface border border-input rounded-xl text-foreground text-sm placeholder:text-muted-foreground focus:border-primary focus:outline-none pr-16" placeholder="0.00" data-testid="trade-amount-input" />
              ...
              <button onClick={initiateTrade} disabled={initiating} className="w-full py-3.5 bg-success text-success-foreground font-bold rounded-xl hover:opacity-90 transition-opacity disabled:opacity-50" data-testid="initiate-trade-btn">
